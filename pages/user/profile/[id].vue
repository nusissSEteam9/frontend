<template>
    <div class="container">
        ...
        <div style="display: flex;align-items: center;margin-bottom: 15px;">
            <img src="/user.png" alt="User Image" style="width: 100px; height: 100px; margin-bottom: 15px;margin-right: 15px;" >
            <div style="display: flex;flex-direction: column;">
                <h2><b>{{ member.username }}</b></h2>
                <p>UID: {{ member.id }}</p>
            </div>
        </div>
        <button v-if="!isAdmin" class="btn btn-danger" @click="reportMember(member.id)">Report User</button>
        <br><br><br>
        ...
        <div style="text-align: center;">
            ...
            <div v-if="totalPages > 1" class="pagination">
                <button type="button" :disabled="currentPage === 1" class="btn btn-outline-primary" @click="prevPage">&laquo;</button>
                <button v-for="page in pagesToShow" :key="page" type="button" class="btn btn-outline-secondary" :class="{ active: page === currentPage }" @click="goToPage(page)">
                    {{ page }}
                </button>
                <button :disabled="currentPage === totalPages" type="button" class="btn btn-outline-primary" @click="nextPage">&raquo;</button>
            </div>
        </div>
    </div>
</template>
