<template>
    <div class="container mt-5">
        ...
        <div class="d-flex justify-content-end mb-4">
            <button class="btn btn-primary" @click="goToCreateRecipe">Add Recipe</button>
        </div>
        ...
        <div v-if="recipes.length > 0" class="row g-4">
            <div v-for="recipe in recipes" :key="recipe.id" class="col-md-3">
                ...
                <div class="card h-100">
                    ...
                    <div class="card-body">
                        <h5 class="card-title">
                            <a :href="`/recipe/detail/${recipe.id}`">{{ recipe.name }}</a>
                        </h5>
                    ...
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item d-flex justify-content-around">
                            <button class="btn btn-outline-secondary btn-sm"
                                @click="goToEditRecipe(recipe.id)">Edit</button>
                            <button class="btn btn-outline-danger btn-sm"
                                @click="promptDelete(recipe.id)">Delete</button>
                        </li>
                    </ul>
                </div>
            </div>
...
onMounted(async () => {
    try {
        const response = await fetch('/api/member/myRecipeList')
        if (response.ok) {
